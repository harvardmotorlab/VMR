...
vstop_thresh=300;
singletravthresh=345;   % movement must be at least 7.6 cm long  (control the distance from the start point of every trial)
singletravthresh_NA=200;   %Generalization block,red ring without target, get the directon of cursor not the cursor reach the red ring
...
hitcirc_count=1;
insidecircle=0;
outsidecircle=0;          %generalization block: cursor move further than the raduis of target, such that cursor can reach target
started=0;
ismoving=0;
firsttimeout=0;
ii=2;
...
while ~any(buttons(2)) &&ii<length(philist)    %~any(buttons(2:end)), the button of corsor is not pressed,the button is 3 dim vector[~,~,~]which respresents left, mid and right button of corsor. if the rigjt botton is not pressed, the code in which is goning on
    ...
    if started==0    % inside start circle wait time (not yet started)
        ...
        if curtime <= t_a + wait_time2 && ~ismember(movement_type(ii),nfbret)
            ...
            %  There are 15 blocks 'a'--'o', 'a'-'b' are the familarization block, 'c'-'e'are the baseline blocks,'h','i','l','m' are the training block.
            % 'f','g','j','k','n' and 'o' are the generalization block. In 'f','k' and 'n', the red ring is selected by subjects, but in 'g','j' and 'o', the red ring is fixed and the target (blue circle) is replaced by the red ring
            % 'c' and 'd' the red ring is fixed with angle (0 or 30 degree)from target. In training block of ,'h','i','l','m', the red ring have to been selected by subjects, added shengxin 9.23.2014
            
            if tgt_set~='f'&&tgt_set~='g'&&tgt_set~='j'&&tgt_set~='k'&&tgt_set~='n'&&tgt_set~='o'
...
%                     end
                ...
                elseif tgt_set=='e'||tgt_set=='h'||tgt_set=='i'||tgt_set=='l'|| tgt_set=='m'||tgt_set=='p'|| tgt_set=='q'||tgt_set=='r'|| tgt_set=='s'
%                     disp('red ring')
                    cursor_tgt_angle = atan2( diff(y1list(ii-1:ii)), diff(x1list(ii-1:ii)) );
                    %%load the aiming target position of last trial
                    if ~aiming_flag && (mod(ii,2)==0)
                        if ii==2&&tgt_set~='e'&&tgt_set~='h'  % download the position of aiming target in last block
                            load('aimingtag_out.mat')
                            testangle_out=testangle_out;
                        elseif ii==26&&tgt_set=='h'
...
%                             aimingangle_out=aimingangle1(1:end-1);     %delete the last zero in the vector
            ...
            % %%======================================================
            %%general learning block with aiming target
            if  tgt_set=='f'||tgt_set=='k'||tgt_set=='n'
                if mod(ii,2)==0
                    while ~any(curkey1==uparrow) && curkey~=1
                        [keyIsDown,keySecs,keyCode] = KbCheck;
                        [mx, my, buttons] = GetMouse;
...
%                 end
            ...
            %%=========================================================================
             % make different type of target displayed at p1
            if tgt_set~='g'&&tgt_set~='j'&&tgt_set~='o'           % Fam, Base, trian and Gen-A blocks, target is a circle
                Screen('DrawTexture', win, circletext,[],p1loc,[],[],[],nextcol);
                Screen('DrawTexture', win, circletext,[],screenoffset-p1loc(1,[3,4,1,2]),[],[],[],nextcol);
            elseif  tgt_set=='g'||tgt_set=='j'||tgt_set=='o'      %% target in NA-Gen block is changed into a red ring
                if mod(ii,2)==0
           ...
           %%when distance of cursor is larger than singletravthresh,next target will appear in the center of the screen
            %==============================================================
            if tgt_set=='g'|| tgt_set=='j'|| tgt_set=='o'
            ...
            elseif  tgt_set=='f'|| tgt_set=='k'|| tgt_set=='n'
            ...
            %%%================== cursor feedback of trials in Fam, Baseline and Train blocks ==================================
            ...
            else   %================== no cursor feedback of trials of Gen-block ==================================
    ...
    if tgt_set=='a'||tgt_set=='b'  %familarization: cursor feedback both in inward and outward movements
    ...
    else    %%Baseline and train: cursor feedback in outward movements and no cursor feedback in inward movements
        if tgt_set~='f'&&tgt_set~='g'&&tgt_set~='j'&&tgt_set~='k'&&tgt_set~='n'&&tgt_set~='o'
            if mod(ii,2)==0
                if (vis_feedback(ii)==1 && (jumpthegun==0 && firsttimeout ==1) || sqrt((dx-rotx)^2 + (dy-roty)^2) < startvisthresh) || ((ismember(movement_type(ii),nfbret) || started==0) && sqrt((dx-rotx)^2 + (dy-roty)^2) < startvisthresh) || (~ismember(movement_type(ii),nfbret) && ismoving==0)
                    Screen('DrawTexture', win, cursortext,[],cursor,[],[],[],cursorcol);
        ...
        else   %%Gen blocks: no cursor feedback both in inward and outward movements 
