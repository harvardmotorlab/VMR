function runexp_strategy_VMR_Exp_grad(name_prefix,tgt_file_name_prefix,tgt_set)
    ...
    if started==0    % inside start circle wait time (not yet started)
        ...
        if curtime <= t_a + wait_time2 && ~ismember(movement_type(ii),nfbret)
            ...
            if tgt_set=='c'||tgt_set=='d'   %if code used for test the baseline block, need to comment this line
              ...
              elseif tgt_set=='e'||tgt_set=='f'||tgt_set=='g'||tgt_set=='h'|| tgt_set=='i'||tgt_set=='j'||tgt_set=='k'||tgt_set=='l'|| tgt_set=='m'||tgt_set=='n'||tgt_set=='o'||tgt_set=='p'||tgt_set=='q'
