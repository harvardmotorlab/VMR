...
while ~any(buttons(2)) &&ii<length(philist)    %~any(buttons(2:end)), the button of corsor is not pressed,the button is 3 dim vector[~,~,~]which respresents left, mid and right button of corsor. if the rigjt botton is not pressed, the code in which is goning on
    ...
    if started==0    % inside start circle wait time (not yet started)
        ...
        if curtime <= t_a + wait_time2 && ~ismember(movement_type(ii),nfbret)
           ...
           %%show many small red dots on the screen           
            for i=1:180
            ...
            %%There are 17 blocks 'a'--'q', 'a'-'b' are the familarization block, 'c'-'e'are the baseline blocks,'f'--'q'are the training block.'r' and 's'are the washout block, if subjects run so fast that they will be tested in washout block
            %%'c' and 'd' the red ring is fixed with angle (0 or 30 degree)from target. In 'e'--'q' or 'r','s', the red ring have to been selected by subjects, added shengxin 9.23.2014
            ...
            elseif tgt_set=='e'||tgt_set=='f'||tgt_set=='g'||tgt_set=='h'|| tgt_set=='i'||tgt_set=='j'|| tgt_set=='k'||tgt_set=='l'|| tgt_set=='m'||tgt_set=='n'|| tgt_set=='o'||tgt_set=='p'|| tgt_set=='q'|| tgt_set=='r'
                cursor_tgt_angle = atan2( diff(y1list(ii-1:ii)), diff(x1list(ii-1:ii)) );
    ...
    %%======== In this experiment, there is no cursor feedback inward movement in the baseline and train section 'c'--'s'. However, there are cursor feedback
    %%======== both in outward and inward movements in the familarization blocks 'a'and 'b'.
    ...
    elseif tgt_set=='c'||tgt_set=='d'||tgt_set=='e'||tgt_set=='f'||tgt_set=='g'||tgt_set=='h'|| tgt_set=='i'||tgt_set=='j'|| tgt_set=='k'||tgt_set=='l'|| tgt_set=='m'||tgt_set=='n'|| tgt_set=='o'||tgt_set=='p'|| tgt_set=='q'|| tgt_set=='r'
        if mod(ii,2)==0
            if (vis_feedback(ii)==1 && (jumpthegun==0 && firsttimeout ==1) || sqrt((dx-rotx)^2 + (dy-roty)^2) < startvisthresh) || ((ismember(movement_type(ii),nfbret) || started==0) && sqrt((dx-rotx)^2 + (dy-roty)^2) < startvisthresh) || (~ismember(movement_type(ii),nfbret) && ismoving==0)
                Screen('DrawTexture', win, cursortext,[],cursor,[],[],[],cursorcol);
                Screen('DrawTexture', win, cursortext,[],screenoffset-cursor(1,[3,4,1,2]),[],[],[],nofeedcursorcol);
