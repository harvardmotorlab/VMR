...
aim_rotation = psychtest(:,13);    %psychtest(:,10) is the position of aiming target in the training block
...
while ~any(buttons(2)) &&ii<length(philist)    %~any(buttons(2:end)), the button of corsor is not pressed,the button is 3 dim vector[~,~,~]which respresents left, mid and right button of corsor. if the rigjt botton is not pressed, the code in which is goning on
        ...
        if tgt_set~='a'&&tgt_set~='b'
            ...
            if aim_fix_base(ii)==777
                ...
                elseif mod(ii,2)==0&&philist(ii)~=0
                    aim_tgt_x_out=xstart+movelen*sind(aim_rotation(ii)+cursor_tgt_angle);
                    aim_tgt_y_out=ystart+movelen*cosd(aim_rotation(ii)+cursor_tgt_angle);
                ...
                elseif mod(ii,2)~=0&&philist(ii)~=0
                    aim_tgt_x_back=xstart-movelen*sind(aim_rotation(ii)+cursor_tgt_angle);
                    aim_tgt_y_back=ystart-movelen*cosd(aim_rotation(ii)+cursor_tgt_angle);
                ...
                %training block of exp,a with fixed aimingtarget
            elseif aim_fix_base(ii)~=777&&aim_fix_train(ii)==888
                if  mod(ii,2)==0  %show the aiming target when it is fixed at 30 or -30
                    aim_tgt_x_out=xstart+movelen*sind(aim_rotation(ii));
                    aim_tgt_y_out=ystart+movelen*cosd(aim_rotation(ii));
                    testcursor1_out=[aim_tgt_x_out-targetsize aim_tgt_y_out-targetsize aim_tgt_x_out+targetsize aim_tgt_y_out+targetsize];
                    Screen('DrawTexture', win, targtext,[],testcursor1_out,[],[],[],sel_aimingtargcol);
                    Screen('DrawTexture', win, targtext,[],screenoffset-testcursor1_out(1,[3,4,1,2]),[],[],[],sel_aimingtargcol);
                elseif mod(ii,2)~=0
                    aim_tgt_x_back=xstart-movelen*sind(aim_rotation(ii));
                    aim_tgt_y_back=ystart-movelen*cosd(aim_rotation(ii));
                    testcursor1_back=[aim_tgt_x_back-targetsize aim_tgt_y_back-targetsize aim_tgt_x_back+targetsize aim_tgt_y_back+targetsize];
                    Screen('DrawTexture', win, targtext,[],testcursor1_back,[],[],[],sel_aimingtargcol);
                    Screen('DrawTexture', win, targtext,[],screenoffset-testcursor1_back(1,[3,4,1,2]),[],[],[],sel_aimingtargcol);
                end
            ...
            if mod(ii,2)==0
                ...
                if aim_tgt_x_out<xstart&&aim_tgt_y_out>ystart
                    angle_tag_out=atan((aim_tgt_y_out-ystart)/(xstart-aim_tgt_x_out))-pi/2;
                elseif aim_tgt_x_out<xstart&&aim_tgt_y_out<ystart
                    angle_tag_out=-atan((ystart-aim_tgt_y_out)/(xstart-aim_tgt_x_out))-pi/2;
                elseif aim_tgt_x_out>xstart&&aim_tgt_y_out>ystart
                    angle_tag_out=-atan((aim_tgt_y_out-ystart)/(aim_tgt_x_out-xstart))+pi/2;
                elseif aim_tgt_x_out>xstart&&aim_tgt_y_out<ystart
                    angle_tag_out=atan((ystart-aim_tgt_y_out)/(aim_tgt_x_out-xstart))+pi/2;
                elseif aim_tgt_y_out==ystart&&aim_tgt_x_out<xstart
                    angle_tag_out=-pi/2;
                elseif aim_tgt_y_out==ystart&&aim_tgt_x_out>xstart
                    angle_tag_out=pi/2;
                elseif aim_tgt_x_out==xstart
                    angle_tag_out=0;
                end
            ...
            if mod(ii,2)~=0
                ...
                % save the  position of the aiming target
                if aim_tgt_x_back<xstart&&aim_tgt_y_back>ystart
                    angle_tag_back=atan((aim_tgt_y_back-ystart)/(xstart-aim_tgt_x_back))-pi/2;
                elseif aim_tgt_x_back<xstart&&aim_tgt_y_back<ystart
                    angle_tag_back=-atan((ystart-aim_tgt_y_back)/(xstart-aim_tgt_x_back))-pi/2;
                elseif aim_tgt_x_back>xstart&&aim_tgt_y_back>ystart
                    angle_tag_back=-atan((aim_tgt_y_back-ystart)/(aim_tgt_x_back-xstart))+pi/2;
                elseif aim_tgt_x_back>xstart&&aim_tgt_y_back<ystart
                    angle_tag_back=atan((ystart-aim_tgt_y_back)/(aim_tgt_x_back-xstart))+pi/2;
                elseif aim_tgt_y_back==ystart&&aim_tgt_x_back<xstart
                    angle_tag_back=-pi/2;
                elseif aim_tgt_y_back==ystart&&aim_tgt_x_back>xstart
                    angle_tag_back=pi/2;
                elseif aim_tgt_x_back==xstart
                    angle_tag_back=0;
                end
    ...
    if exp_type(ii)==0
        if tgt_set~='a'&&tgt_set~='b'
            %Baseline and training block of B and C with slected aimingtarget
            if aim_fix_train(ii)==888
                if  mod(ii,2)==0  %show the aiming target when it is fixed at 30 or -30
                    testcursorx=xstart+movelen*sind(aim_rotation(ii));
                    testcursory=ystart+movelen*cosd(aim_rotation(ii));
                    testcursor1_out=[testcursorx-targetsize testcursory-targetsize testcursorx+targetsize testcursory+targetsize];
                    Screen('DrawTexture', win, targtext,[],testcursor1_out,[],[],[],sel_aimingtargcol);
                    Screen('DrawTexture', win, targtext,[],screenoffset-testcursor1_out(1,[3,4,1,2]),[],[],[],sel_aimingtargcol);
                elseif mod(ii,2)~=0
                    testcursorx=xstart-movelen*sind(aim_rotation(ii));
                    testcursory=ystart-movelen*cosd(aim_rotation(ii));
                    testcursor1_back=[testcursorx-targetsize testcursory-targetsize testcursorx+targetsize testcursory+targetsize];
                    Screen('DrawTexture', win, targtext,[],testcursor1_back,[],[],[],sel_aimingtargcol);
                    Screen('DrawTexture', win, targtext,[],screenoffset-testcursor1_back(1,[3,4,1,2]),[],[],[],sel_aimingtargcol);
                end
            elseif aim_fix_train(ii)~=888&&aim_selection(ii)==999    %show how to select the aiming target in the baseline block and training block
            ...
            if ii==length(x1list)-2
                ...
                save(savefile, 'testcursorx', 'testcursory','testangle_out');
            end
            ...
            if ii==length(x1list)-1
                ...
                save(savefile, 'testcursorx', 'testcursory','testangle_back');
            end
        end
    end
        %=======================================================================
        ...
        if started==0    % inside start circle wait time (not yet started)
            ...
            if curtime <= t_a + wait_time2 && ~ismember(movement_type(ii),nfbret)
                ...
                if (insidecircle==1 && curtime-t1 >= timeinside && ismoving == 1) || (ismember(movement_type(ii),ptlist) && (curtime-movement_start_time > maxsingletime || (max(total_vel((k-6):k)) < vstop_thresh && sqrt((dx-rotx)^2 + (dy-roty)^2) > singletravthresh)))
                    ...
                    %%==========================================================
                    %                         %add code for avoid cursor directly move to cursor target
                    %                         if philist(ii)==0&&aim_type(ii)==999
                    %                             theta=atan((dy-ystart)/(dx-xstart));
                    %                             theta1=atan(30/(y1list(2)-y1list(1)));
                    %                             if sqrt(((dx-centercircle_x1)^2+(dy-centercircle_y1)^2))>2*circlesize&&(theta<theta1||theta>-theta1)
                    %                                 Snd('Play',glass,fs);
                    %                             end
                    %                         end
