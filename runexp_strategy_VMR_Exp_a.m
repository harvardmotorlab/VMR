function runexp_strategy_VMR_Exp_a(name_prefix,tgt_file_name_prefix,tgt_set)
...
aim_fix_base = psychtest(:,11);        %psychtest(:,11) is the mark for different block: Familiration, baseline or training
aim_fix_train = psychtest(:,12);        %psychtest(:,11) is the mark for different block: Familiration, baseline or training
aim_rotation = psychtest(:,13)*pi/180;    %psychtest(:,10) is the position of aiming target in the training block
aim_selection = psychtest(:,14);    %psychtest(:,10) is the position of aiming target in the training block
...
while ~any(buttons(2)) &&ii<length(philist)    %~any(buttons(2:end)), the button of corsor is not pressed,the button is 3 dim vector[~,~,~]which respresents left, mid and right button of corsor. if the rigjt botton is not pressed, the code in which is goning on
    ...
    if started==0    % inside start circle wait time (not yet started)
        ...
        if curtime <= t_a + wait_time2 && ~ismember(movement_type(ii),nfbret)
            ...
            % This experiment has 11 blocks 'a'---'k', Fam:'a' and 'b', Baseline:'c','d' and 'e'. In baseline 'c'and 'd', the red ring is fixed in line with the cursor target or off the
            % target with 30 degree. Train blocks: 'f'--'k'. In train 'f',
            % first 12 trials, red ring is fixed and the red ring could be selected by subjects from 25th trial. In blocks 'g'--'k', the red ring is selected by subjects
            % show the aiming target after the cursor reaches the "S", this block is aiming target without being selected by subjects
            if tgt_set=='c'||tgt_set=='d'   % baseline blocks, red ring is fixed
                %baseline block of exp,a with fixed aimingtarget
                if aim_fix_base(ii)==777
                    cursor_tgt_angle = atan2( diff(y1list(ii-1:ii)), diff(x1list(ii-1:ii)) );   %caculate the angle of red ring displayed  
                    if mod(ii,2)==0&&philist(ii)==0                                              %if rotation is zero, the red ring is inline with target
                    ...
                    elseif mod(ii,2)==0&&philist(ii)~=0                                          %if rotation is 30 degree, the red ring is off the target
                ...
                if mod(ii,2)==0
                    aimingtagpos_x_out(ii-1)=aim_tgt_x_out;                                                  %save the red ring x and y cordinate
                    aimingtagpos_y_out(ii-1)=aim_tgt_y_out;
                    aimingangle1(t)=save_angle_out(aim_tgt_x_out,aim_tgt_y_out,xstart,ystart);               %save the angle of red ring
                end
                if aimingangle1(t)==2                         % save the position of the cursor witho 0 in the vector
                    t=t-1;
                end
                if ii==length(x1list)-2
                    aimingangle_out=aimingangle1(1:end-1);     %delete the last zero in the vector of red ring's angle
                end
                %%%move_back direction
                if mod(ii,2)~=0
                    aimingtagpos_x_back(ii-1)=aim_tgt_x_back;
                    aimingtagpos_y_back(ii-1)=aim_tgt_y_back;
            ...
            elseif tgt_set=='e'||tgt_set=='f'||tgt_set=='g'||tgt_set=='h'||tgt_set=='i'||tgt_set=='j'||tgt_set=='k'  
                ...
                if ~aiming_flag && (mod(ii,2)==0)
                    ...
                    elseif ii==2&&tgt_set=='e'&&tgt_set=='f'    %%in third baseline block or first train block, the red ring is fixed on top of the target
                        testangle_out=0;
                    end
%                     disp('Loading aiming target!!!')
                    aiming_flag = 1;
                elseif ~aiming_flag
                    ...
                    elseif ii==3&&tgt_set=='e'&&tgt_set=='f'
                        testangle_back=0;
                    end
                    aiming_flag = 1;
                end
                
                if  tgt_set=='f'&&aim_selection(ii)~=999     %%in the first 24 trial of first train block, the red ring is fixed
                ...
                elseif aim_selection(ii)==999      %After 25th trial, the red ring is selected by subjects
                    ...
                    while ~any(curkey1==uparrow) && curkey~=1
                        ...
                        if keyaim % if using key to aim
                            ...
                            if keyIsDown
                                ...
                                else             %move_back left and right key are reversed to move the red ring
