...
while ~any(buttons(2)) &&ii<length(philist)    %~any(buttons(2:end)), the button of corsor is not pressed,the button is 3 dim vector[~,~,~]which respresents left, mid and right button of corsor. if the rigjt botton is not pressed, the code in which is goning on
    ...
    if started==0    % inside start circle wait time (not yet started)
        ...
        if curtime <= t_a + wait_time2 && ~ismember(movement_type(ii),nfbret)
            ...
            %  There are 19 blocks 'a'--'q', 'a'-'b' are the familarization block, 'c'-'e'are the baseline blocks,'h'--'k' and 'n'--'q' are the training block.
            % 'f','g','l','m','r' and 's' are the generalization block. In
            % 'f','m' and 'r', the red ring is selected by subjects, but in 'g','l' and 's', the red ring is fixed and the target (blue circle) is replaced by the red ring
            % 'c' and 'd' the red ring is fixed with angle (0 or 30 degree)from target. In training block of 'h'--'k' and 'n'--'q', the red ring have to been selected by subjects, added shengxin 9.23.2014
           ...
           %%============== instructiion of this section about these blocks===============================================================
           %%show the target and red ring (selected by subjects), in the trials of train or baseline blocks that there are cursor feedback both inward and outward movements
                elseif ((tgt_set=='e'||tgt_set=='h'||tgt_set=='i'|| tgt_set=='j'||tgt_set=='k'||tgt_set=='n'||tgt_set=='o'|| tgt_set=='p'|| tgt_set=='q'|| tgt_set=='t'|| tgt_set=='u')&&(cursor_nofeedback(ii)~=0))                      
                    cursor_tgt_angle = atan2( diff(y1list(ii-1:ii)), diff(x1list(ii-1:ii)) );
                    ...
                    if ~aiming_flag && (mod(ii,2)==0)
                        ...
                        elseif ii==2&&(tgt_set=='i'|| tgt_set=='j'||tgt_set=='n'|| tgt_set=='t'|| tgt_set=='u')     % download the position of aiming target in last block
            ...
            % %%======================================================
            %%show the target and red ring (selected by subjects), in the trials of train or generalization blocks selecting red ring that there are no cursor feedback both inward and outward movements  
            %%general learning block with selecting aiming target
            if  (tgt_set=='f'||tgt_set=='m'||tgt_set=='r')||((tgt_set=='c'||tgt_set=='d'||tgt_set=='e'||tgt_set=='h'||tgt_set=='i'|| tgt_set=='j'||tgt_set=='k'||tgt_set=='n'||tgt_set=='o'|| tgt_set=='p'|| tgt_set=='q'|| tgt_set=='t'|| tgt_set=='u')&& (cursor_nofeedback(ii)==0))
                ...
                if (tgt_set=='f'||tgt_set=='m'||tgt_set=='r')||((tgt_set=='e'||tgt_set=='h'||tgt_set=='i'|| tgt_set=='j'||tgt_set=='k'||tgt_set=='n'||tgt_set=='o'|| tgt_set=='p'|| tgt_set=='q'|| tgt_set=='t'|| tgt_set=='u')&& (cursor_nofeedback(ii)==0))
                    ...
                    if mod(ii,2)==0
                        while ~any(curkey1==uparrow) && curkey~=1
                            ...
                            if tgt_set=='e'
                                testcursorx_out=xstart+movelen*cos(testangle_out+cursor_tgt_angle_base );
                                testcursory_out=ystart+movelen*sin(testangle_out+cursor_tgt_angle_base );     %%red ring shown in the equal division point every time
                            elseif  (tgt_set=='f'||tgt_set=='m'||tgt_set=='r')
                            ...
                            elseif ((tgt_set=='h'||tgt_set=='i'||tgt_set=='j'||tgt_set=='k'||tgt_set=='n'||tgt_set=='o'||tgt_set=='p'|| tgt_set=='q'|| tgt_set=='t'|| tgt_set=='u'))
                                testcursorx_out=xstart+movelen*cos(testangle_out);
            ...
            %%=========================================================================
            % make circle at p1
            if tgt_set~='g'&&tgt_set~='l'&&tgt_set~='s'            %other blocks, target is a circle
                Screen('DrawTexture', win, circletext,[],p1loc,[],[],[],nextcol);
                Screen('DrawTexture', win, circletext,[],screenoffset-p1loc(1,[3,4,1,2]),[],[],[],nextcol);
            elseif  tgt_set=='g'||tgt_set=='l'||tgt_set=='s'      %% target in NA-Gen block is changed into a red ring
                if mod(ii,2)==0
            ...
            % make word 'S' displayed at start location
            if ~ismember(movement_type(ii),nfbret)
                Screen('DrawText', win, 'S', xstart-7, ystart-14,[255 255 255]);              %draw the start point as "S"
                Screen('DrawTexture', win, circletext,[],screenoffset-startloc(1,[3,4,1,2]),[],[],[],startcol_user);
            else
            ...
            %===================control the distance of cursor in outward movement of Gen block ===========================================
            %%whne distance of cursor is larger than singletravthresh,next target will appear in the center of the screen
            %==============================================================
            if tgt_set=='g'|| tgt_set=='l'|| tgt_set=='s'   %%Aiming target is fixed
            ...
            elseif (tgt_set=='f'||tgt_set=='m'||tgt_set=='r')||((tgt_set=='c'||tgt_set=='d'||tgt_set=='e'||tgt_set=='h'||tgt_set=='i'|| tgt_set=='j'||tgt_set=='k'||tgt_set=='n'||tgt_set=='o'|| tgt_set=='p'|| tgt_set=='q'|| tgt_set=='t'|| tgt_set=='u')&& (cursor_nofeedback(ii)==0))  
            ...
            %==============================================================
            %%Because of some mistake, the increse of 'ii' of feedbakc trials of familiarization, baseline, training and generalization block are divided into two section. 
            %%%================== cursor feedback of trials outward movements ==================================
            % inside circle for long enough and then move on to next movement
            if  ( tgt_set=='a'||tgt_set=='b')||((tgt_set=='c'||tgt_set=='d'||tgt_set=='e'||tgt_set=='h'||tgt_set=='i'|| tgt_set=='j'||tgt_set=='k'||tgt_set=='n'||tgt_set=='o'|| tgt_set=='p'|| tgt_set=='q'|| tgt_set=='t'|| tgt_set=='u')&& (cursor_nofeedback(ii)~=0))
                ...
                %%%==================no cursor feedback of trials outward movements==================================
            elseif (tgt_set=='f'|| tgt_set=='g'|| tgt_set=='l'||tgt_set=='m'|| tgt_set=='r'|| tgt_set=='s')||((tgt_set=='c'||tgt_set=='d'||tgt_set=='e'||tgt_set=='h'||tgt_set=='i'|| tgt_set=='j'||tgt_set=='k'||tgt_set=='n'||tgt_set=='o'|| tgt_set=='p'|| tgt_set=='q'|| tgt_set=='t'|| tgt_set=='u')&& (cursor_nofeedback(ii)==0))
    ...
    %%%%%%%%%%%%%%===========================================================
    if tgt_set=='a'||tgt_set=='b'   %%familarization: cursor feedback both in inward and outward movements
    ...
    else    %%Baseline and train: cursor feedback both in inward and outward movements with sign of cursor feedback in outward movements
        if ((tgt_set=='c'||tgt_set=='d'||tgt_set=='e'||tgt_set=='h'||tgt_set=='i'|| tgt_set=='j'||tgt_set=='k'||tgt_set=='n'||tgt_set=='o'|| tgt_set=='p'|| tgt_set=='q'|| tgt_set=='t'|| tgt_set=='u')&& (cursor_nofeedback(ii)~=0))
            if mod(ii,2)==0
                if (vis_feedback(ii)==1 && (jumpthegun==0 && firsttimeout ==1) || sqrt((dx-rotx)^2 + (dy-roty)^2) < startvisthresh) || ((ismember(movement_type(ii),nfbret) || started==0) && sqrt((dx-rotx)^2 + (dy-roty)^2) < startvisthresh) || (~ismember(movement_type(ii),nfbret) && ismoving==0)
                    Screen('DrawTexture', win, cursortext,[],cursor,[],[],[],cursorcol);
                    Screen('DrawTexture', win, cursortext,[],screenoffset-cursor(1,[3,4,1,2]),[],[],[],nofeedcursorcol);
        ...
        %%Baseline and train: cursor feedback both in inward and outward movements with sign of no cursor feedback(in tgt.file) in outward movements
        elseif (tgt_set=='f'|| tgt_set=='g'|| tgt_set=='l'||tgt_set=='m'|| tgt_set=='r'|| tgt_set=='s')||((tgt_set=='c'||tgt_set=='d'||tgt_set=='e'||tgt_set=='h'||tgt_set=='i'|| tgt_set=='j'||tgt_set=='k'||tgt_set=='n'||tgt_set=='o'|| tgt_set=='p'|| tgt_set=='q'|| tgt_set=='t'|| tgt_set=='u')&& (cursor_nofeedback(ii)==0))          
