...
while ~any(buttons(2)) &&ii<length(philist)    %~any(buttons(2:end)), the button of corsor is not pressed,the button is 3 dim vector[~,~,~]which respresents left, mid and right button of corsor. if the rigjt botton is not pressed, the code in which is goning on
    ...
    if started==0    % inside start circle wait time (not yet started)
        ...
        if curtime <= t_a + wait_time2 && ~ismember(movement_type(ii),nfbret)
            ...
            if tgt_set~='f'&&tgt_set~='g'&&tgt_set~='m'&&tgt_set~='n'&&tgt_set~='s'&&tgt_set~='t'
                ...
                elseif (tgt_set=='h')||((tgt_set=='e'||tgt_set=='i'||tgt_set=='j'|| tgt_set=='k'||tgt_set=='l'||tgt_set=='o'||tgt_set=='p'|| tgt_set=='q'|| tgt_set=='r')&&(cursor_nofeedback(ii)~=0))  %%insert 3 trials in all training block
                      
                    cursor_tgt_angle = atan2( diff(y1list(ii-1:ii)), diff(x1list(ii-1:ii)) );
                    %%load the aiming target position of last trial
                    if ~aiming_flag && (mod(ii,2)==0)
                        if (tgt_set=='i'||tgt_set=='h')&& ii==2
                        ...
                        elseif ii==2&&(tgt_set=='j'|| tgt_set=='k'||tgt_set=='l'|| tgt_set=='o'||tgt_set=='p'||tgt_set=='q'|| tgt_set=='r')     % download the position of aiming target in last block
            ...
            if  (tgt_set=='g'||tgt_set=='m'||tgt_set=='t')||((tgt_set=='c'||tgt_set=='d'||tgt_set=='e'||tgt_set=='i'||tgt_set=='j'|| tgt_set=='k'||tgt_set=='l'||tgt_set=='o'||tgt_set=='p'|| tgt_set=='q'|| tgt_set=='r')&& (cursor_nofeedback(ii)==0))
                ...
                if (tgt_set=='g'||tgt_set=='m'||tgt_set=='t')||((tgt_set=='e'||tgt_set=='i'||tgt_set=='j'|| tgt_set=='k'||tgt_set=='l'||tgt_set=='o'||tgt_set=='p'|| tgt_set=='q'|| tgt_set=='r')&& (cursor_nofeedback(ii)==0))
                    
                    if ~aiming_flag && (mod(ii,2)==0)
                        if ii==2&&((tgt_set=='l'||tgt_set=='p'||tgt_set=='q'|| tgt_set=='r'))  % download the position of aiming target in last block
                            load('aimingtag_out.mat')
                            testangle_out=testangle_out;
                        end
                        aiming_flag = 1;
                    ...
                    if mod(ii,2)==0
                        while ~any(curkey1==uparrow) && curkey~=1
                            ...
                            if tgt_set=='e'
                                testcursorx_out=xstart+movelen*cos(testangle_out+cursor_tgt_angle_base );
                                testcursory_out=ystart+movelen*sin(testangle_out+cursor_tgt_angle_base );     %%red ring shown in the equal division point every time
                            elseif  (tgt_set=='g'||tgt_set=='m'||tgt_set=='t')
                            ...
                            elseif ((tgt_set=='l'||tgt_set=='p'||tgt_set=='q'|| tgt_set=='r'))
                                testcursorx_out=xstart+movelen*cos(testangle_out);
            ...
            %%=========================================================================
            % make circle at p1
            if tgt_set~='f'&&tgt_set~='n'&&tgt_set~='s'            %other blocks, target is a circle
                Screen('DrawTexture', win, circletext,[],p1loc,[],[],[],nextcol);
                Screen('DrawTexture', win, circletext,[],screenoffset-p1loc(1,[3,4,1,2]),[],[],[],nextcol);
            elseif  tgt_set=='f'||tgt_set=='n'||tgt_set=='s'      %% target in NA-Gen block is changed into a red ring
                if mod(ii,2)==0
            ...
            %==============================================================
            if tgt_set=='f'|| tgt_set=='n'|| tgt_set=='s'   %%Aiming target is fixed
            ...
            elseif (tgt_set=='g'||tgt_set=='m'||tgt_set=='t')||((tgt_set=='c'||tgt_set=='d'||tgt_set=='e'||tgt_set=='i'||tgt_set=='j'|| tgt_set=='k'||tgt_set=='l'||tgt_set=='o'||tgt_set=='p'|| tgt_set=='q'|| tgt_set=='r')&& (cursor_nofeedback(ii)==0))  
            ...
            if  ( tgt_set=='a'||tgt_set=='b'||tgt_set=='h')||((tgt_set=='c'||tgt_set=='d'||tgt_set=='e'||tgt_set=='i'||tgt_set=='j'|| tgt_set=='k'||tgt_set=='l'||tgt_set=='o'||tgt_set=='p'|| tgt_set=='q'|| tgt_set=='r')&& (cursor_nofeedback(ii)~=0))
            ...
            elseif (tgt_set=='f'|| tgt_set=='g'|| tgt_set=='m'||tgt_set=='n'|| tgt_set=='s'|| tgt_set=='t')||((tgt_set=='c'||tgt_set=='d'||tgt_set=='e'||tgt_set=='i'||tgt_set=='j'|| tgt_set=='k'||tgt_set=='l'||tgt_set=='o'||tgt_set=='p'|| tgt_set=='q'|| tgt_set=='r')&& (cursor_nofeedback(ii)==0))
    ...
    if tgt_set=='a'||tgt_set=='b'
        ...
        if (tgt_set=='h')||((tgt_set=='c'||tgt_set=='d'||tgt_set=='e'||tgt_set=='i'||tgt_set=='j'|| tgt_set=='k'||tgt_set=='l'||tgt_set=='o'||tgt_set=='p'|| tgt_set=='q'|| tgt_set=='r')&& (cursor_nofeedback(ii)~=0))
            if mod(ii,2)==0
                if (vis_feedback(ii)==1 && (jumpthegun==0 && firsttimeout ==1) || sqrt((dx-rotx)^2 + (dy-roty)^2) < startvisthresh) || ((ismember(movement_type(ii),nfbret) || started==0) && sqrt((dx-rotx)^2 + (dy-roty)^2) < startvisthresh) || (~ismember(movement_type(ii),nfbret) && ismoving==0)
                    Screen('DrawTexture', win, cursortext,[],cursor,[],[],[],cursorcol);
                    Screen('DrawTexture', win, cursortext,[],screenoffset-cursor(1,[3,4,1,2]),[],[],[],nofeedcursorcol);
