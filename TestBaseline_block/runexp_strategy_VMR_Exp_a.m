...
basl_test = psychtest(:,15);         %1 is the test block of baseline
...
while ~any(buttons(2)) &&ii<length(philist)    %~any(buttons(2:end)), the button of corsor is not pressed,the button is 3 dim vector[~,~,~]which respresents left, mid and right button of corsor. if the rigjt botton is not pressed, the code in which is goning on
    ...
    if started==0    % inside start circle wait time (not yet started)
        if ~ismember(movement_type(ii),nfbret)  % if movement is not a no feedback return  movement_type determine the display of "S" 
...
%             if tgt_set=='c'  %if code used for test the baseline block, need to comment this line
               if basl_test(ii)==1      %this line is used to test the long baseline blocks
...
%             elseif tgt_set=='d'||tgt_set=='e'||tgt_set=='f'||tgt_set=='g'||tgt_set=='h'||tgt_set=='i'||tgt_set=='j'
                
                %%load the aiming target position of last trial
                if mod(ii,2)==0
                    if ii==2&&tgt_set~='d'&&tgt_set~='e'&&tgt_set~='j'  % download the position of aiming target in last block
                        load('aimingtag_out.mat')
                        testangle_out=testangle_out;
                    elseif ii==2&&tgt_set=='d'&&tgt_set=='e'&&tgt_set=='j'
                        testangle_out=0;
                    end
                else
                    if ii==3&&tgt_set~='d'&&tgt_set~='e'&&tgt_set~='j'    % download the position of aiming target in last block
                        load('aimingtag_back.mat')
                        testangle_back=testangle_back;
                    elseif ii==3&&tgt_set=='d'&&tgt_set=='e'&&tgt_set=='j'
                        testangle_back=0;
                    end
                end
                
                if  tgt_set=='e'&&aim_selection(ii)~=999
                    cursor_tgt_angle = atan2( diff(y1list(ii-1:ii)), diff(x1list(ii-1:ii)) );
