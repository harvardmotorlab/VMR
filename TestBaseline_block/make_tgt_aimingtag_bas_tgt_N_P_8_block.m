function make_tgt_aimingtag_bas_tgt_N_P(VR)
close all;
% target file format
%   column      function
...
fam_tgt_i = repmat([VR 0 start_x start_y start_x start_y 1 750 0 666 777 0 0 0],2*800,1);
...
% %% find the trail needing the curosr rotation, set the first colom as rotation
% k=1;
% for i=1:1:1600
%     if find(mod(i,10)==2)
%         Num(k)=i;
...
bas_a_tgt1 = [bas_tgt1_int;bas_tgt1_int(49,:);zeros(1,14)];
bas_a_tgt2 = [bas_tgt2_int;bas_tgt2_int(49,:);zeros(1,14)];
bas_a_tgt3 = [bas_tgt3_int;bas_tgt3_int(49,:);zeros(1,14)];
bas_a_tgt4 = [bas_tgt4_int;bas_tgt4_int(49,:);zeros(1,14)];
bas_a_tgt5 = [bas_tgt5_int;bas_tgt5_int(49,:);zeros(1,14)];
bas_a_tgt6 = [bas_tgt6_int;bas_tgt6_int(49,:);zeros(1,14)];
bas_a_tgt7 = [bas_tgt7_int;bas_tgt7_int(49,:);zeros(1,14)];
bas_a_tgt8 = [bas_tgt8_int;bas_tgt8_int(49,:);zeros(1,14)];

%% Combine all movements into a single target set
all_tgt_ = {bas_a_tgt1;bas_a_tgt2;bas_a_tgt3;bas_a_tgt4;bas_a_tgt5;bas_a_tgt6;bas_a_tgt7;bas_a_tgt8};
%% Write tgt files
if sign(VR)>0, ch = 'P'; else ch = 'N'; end
f_prefix = ['vmr_aimingtag_',ch];
if ~isdir([pwd '\tgt_files_aimingtag_base1\']), mkdir([pwd '\tgt_files_aimingtag_base1\']);end
for i = 1:length(all_tgt_)
    dlmwrite([pwd,'\tgt_files_aimingtag_base1\',f_prefix,char('a'+i-1),'.tgt'],all_tgt_{i},' ');
end
%% Plot target files
...
B = [];
for i = 1:4
    F = load([pwd,'\tgt_files_aimingtag_base1\',f_prefix,char('a'+i-1),'.tgt'],'r');
    T = [T;F(2:end-1,:)];
    B = [B;1;0*F(3:end-1,1)];
end;
