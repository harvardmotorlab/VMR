...
while cont && counter<4
    ...
    if cont
        counter = counter + 1;
        if counter<3
            dummy=input(['washout ',int2str(4+counter),': ']);
        else
            dummy=input(['Training',int2str(counter-2),': ']);
        end
        runexp_strategy_VMR_Exp_General_Rot_Wiggle(name_prefix,tgt_prefix,'t'+counter);
    else
        return;
    end;
end
