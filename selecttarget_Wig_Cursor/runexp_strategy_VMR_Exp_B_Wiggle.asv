function runexp_strategy_VMR_Exp_B_Wiggle(name_prefix,tgt_file_name_prefix,tgt_set)
...
aim_fix_train = psychtest(:,12);          %psychtest(:,12) is the mark for training, 888 means red ring is selected by subjected
...
aimingangle2=ones(aiminglen+1,1)*2;             %save the angle of move_back
aimingangle_out=ones(aiminglen-1,1)*2;        %compute the angle of move_out
aimingangle_back=ones(aiminglen-1,1)*2;       %compute the angle of move_back
    ...
    if started==0    % inside start circle wait time (not yet started)
        ...
        if (curtime >= t_a + wait_time) || ismember(movement_type(ii),nfbret)  % in circle for long enough
            ...
            if ~ismember(movement_type(ii),nfbret)  % if it isn't a no feedback return
                Screen('DrawText', win, 'S', xstart-7, ystart-14,[255 255 255]);              %draw the start point as "S"
                %                 Screen('DrawTexture', win, circletext,[],startloc,[],[],[],startcol_exp);
        ...
        if curtime <= t_a + wait_time2 && ~ismember(movement_type(ii),nfbret)
            ...
            % show the aiming target at different positions
            for i=1:180
                targetpost1(i,:)=[testcirclepath(1:2,i)', testcirclepath(1:2,i)'] + [xstart ystart xstart ystart] + ATGT_selection_dot_size*[-1,-1, 1, 1];

            end
            
            %%===========================Instruction of block in experiment================================================
            %  There are 13 blocks 'a'-m', 'a'-'b' are the familarization block, 'c'-'e'are the baseline blocks,'f'-'m' are the training block.
            % 'c' and 'd' the red ring is fixed with angle (0 or 30 degree)from target. In training block of 'f'-'m', the red ring have to been selected by subjects, added shengxin 9.23.2014
            
            %%show the aiming target after the cursor reaches the "S", this block is aiming target without being selected by subjects
            if tgt_set=='c'||tgt_set=='d'                          %In this block, red ring is fexed, subjects don't have to select the red ring.  
                %baseline block of exp,a with fixed aimingtarget
                if aim_fix_base(ii)==777
                    ...
                    if mod(ii,2)~=0&&philist(ii)==0
                        aim_tgt_x_back=xstart+movelen*cos(cursor_tgt_angle);
                        aim_tgt_y_back=ystart+movelen*sin(cursor_tgt_angle);
                        testcursor_back=[aim_tgt_x_back-targetsize aim_tgt_y_back-targetsize aim_tgt_x_back+targetsize aim_tgt_y_back+targetsize];
                        Screen('DrawTexture', win, targtext,[],testcursor_back,[],[],[],sel_aimingtargcol);
                        Screen('DrawTexture', win, targtext,[],screenoffset-testcursor_back(1,[3,4,1,2]),[],[],[],sel_aimingtargcol);
                    elseif mod(ii,2)~=0&&philist(ii)~=0
                        aim_tgt_x_back=xstart+movelen*cos(aim_rotation(ii)+cursor_tgt_angle);
                        aim_tgt_y_back=ystart+movelen*sin(aim_rotation(ii)+cursor_tgt_angle);
                        testcursor1_back=[aim_tgt_x_back-targetsize aim_tgt_y_back-targetsize aim_tgt_x_back+targetsize aim_tgt_y_back+targetsize];
                ...
                if mod(ii,2)==0
                    ...
                    aimingangle1(t)=save_angle_out(aim_tgt_x_out,aim_tgt_y_out,xstart,ystart);
                end
                ...
                %%%move_back direction
                if mod(ii,2)~=0
                    aimingtagpos_x_back(ii-1)=aim_tgt_x_back; aimingtagpos_y_back(ii-1)=aim_tgt_y_back;
                    aimingangle2(num)=save_angle_back(aim_tgt_x_back,aim_tgt_y_back,xstart,ystart);
                end
                aimingangle2(1)=1;
                ...
                if ii==length(x1list)-1
                    Len=length(aimingangle2); aimingangle_back=aimingangle2(2:Len-1);
                end
                
            % In the block in which subjects have to select the aiming target and the cursor appears with the aimingtarget after the cursor reaches "S"
            elseif tgt_set=='e'||tgt_set=='f'||tgt_set=='g'||tgt_set=='h'|| tgt_set=='i'||tgt_set=='j'||tgt_set=='k'

                cursor_tgt_angle = atan2( diff(y1list(ii-1:ii)), diff(x1list(ii-1:ii)) );
                %%load the aiming target position of last trial
                if ~aiming_flag && (mod(ii,2)==0)
                    if ii==2&&tgt_set~='e'&&tgt_set~='f'  % download the position of aiming target in last block
                        load('aimingtag_out.mat')
                        testangle_out=testangle_out;
                    elseif ii==26&&tgt_set=='f'              % set the angle of red ring of trials in training block after many trial of red ring fixed.
                        testangle_out=cursor_tgt_angle;
                    elseif ii==2&&tgt_set=='e'               % set the angle of red ring of first trial in third Baseline block when subjects select red ring.
                        testangle_out=0;                                   %if the cursor has relative position with target, used this line code
                        %                           testangle_out=cursor_tgt_angle;                 %if the cursor is fixed by subject, used this line code
                    end
                    aiming_flag = 1;
                elseif ~aiming_flag && (mod(ii,2)~=0)
                    if ii==3&&tgt_set~='e'&&tgt_set~='f'    % download the position of aiming target in last block
                        load('aimingtag_back.mat')
                        testangle_back=testangle_back;
                    elseif ii==3&&tgt_set=='f'
                        cursor_tgt_angle=-pi/2;
                        testangle_back=cursor_tgt_angle;
                    elseif ii==3&&tgt_set=='e'
                        testangle_back=0;
                        %                         testangle_back=cursor_tgt_angle;
                    end
                    aiming_flag = 1;
                ...
                if  tgt_set=='f'&&aim_selection(ii)~=999                         %%firs 24 trials, the red ring is fixed, subjects should aiming for red ring.
                    cursor_tgt_angle = atan2( diff(y1list(ii-1:ii)), diff(x1list(ii-1:ii)) );
                    if  mod(ii,2)==0  %show the aiming target when it is fixed at 30 or -30
                        ...
                        Screen('DrawTexture', win, targtext,[],screenoffset-testcursor1_out(1,[3,4,1,2]),[],[],[],sel_aimingtargcol);
                    elseif mod(ii,2)~=0
                        testcursorx_back=xstart+movelen*cos(aim_rotation(ii)+cursor_tgt_angle);
                        testcursory_back=ystart+movelen*sin(aim_rotation(ii)+cursor_tgt_angle);
                ...
                elseif aim_selection(ii)==999
                    while ~any(curkey1==uparrow) && curkey~=1
                        ...
                        % select the aiming target by mouse
                        %                         if buttons(1,1)==1
                        %                             curkey=1;
                        %                         end
                        if keyaim % if using key to aim
                            ...
                            if keyIsDown
                                ...
                                if curkey1 == uparrow
                                    curkey1=uparrow;
                                    %                                     break;
                                    %                                   disp('Select aiming target!!!');
                                end

                                if mod(ii,2)==0          %move_out left and right key to move the red ring
                                    ...
                                    %move_back left and right key are reversed to move the red ring
                        ...
                        if mod(ii,2)==0
                            if tgt_set=='e'
                                testcursorx_out=xstart+movelen*cos(testangle_out+cursor_tgt_angle_base );     %%first trial, red ring is in line with target
                                testcursory_out=ystart+movelen*sin(testangle_out+cursor_tgt_angle_base );     %%red ring shown in the equal division point every time(wiggle with cursor target)
                                %                               testcursorx_out=xstart+movelen*cos(testangle_out+cursor_tgt_angle );
                                %                               testcursory_out=ystart+movelen*sin(testangle_out+cursor_tgt_angle );           %keep the relative position between the red ring and target (shown in same place)
                            else
                            ...
                            if tgt_set=='e'                                                                       %%training block the start point of move back is fixed
                                xstart_back=xstart;
                                ystart_back=ystart;
                                testcursorx_back=xstart_back+movelen*cos(testangle_back+cursor_tgt_angle );
                                testcursory_back=ystart_back+movelen*sin(testangle_back+cursor_tgt_angle );            %have the same relative position with target
                            else
                                xstart_back=2890;                                                                      %%training block the start point of move back is fixed, if we want to the red ring has same change as move out we
                                ystart_back=834;                                                                       %%we can change the fixed position to the changing position
                                testcursorx_back=xstart_back+movelen*cos(testangle_back);        % the red ring position is fixed by subjects and same relative position with target
                                testcursory_back=ystart_back+movelen*sin(testangle_back);
                            end
                            testcursor_back=[testcursorx_back-targetsize testcursory_back-targetsize testcursorx_back+targetsize testcursory_back+targetsize];
                            Screen('DrawTexture', win, targtext,[],testcursor_back,[],[],[],sel_aimingtargcol);
                            Screen('DrawTexture', win, targtext,[],screenoffset-testcursor_back(1,[3,4,1,2]),[],[],[],sel_aimingtargcol);
                        end
                    ...
                    if mod(ii,2)==0 %%move_out_direction
                        testcursor1_out=[testcursorx_out-targetsize testcursory_out-targetsize testcursorx_out+targetsize testcursory_out+targetsize];
                        Screen('DrawTexture', win, targtext,[],testcursor1_out,[],[],[],sel_aimingtargcol);
                        Screen('DrawTexture', win, targtext,[],screenoffset-testcursor1_out(1,[3,4,1,2]),[],[],[],sel_aimingtargcol);
                    else %%move_back_direction
                        testcursor1_back=[testcursorx_back-targetsize testcursory_back-targetsize testcursorx_back+targetsize testcursory_back+targetsize];
                        Screen('DrawTexture', win, targtext,[],testcursor1_back,[],[],[],sel_aimingtargcol);
                        Screen('DrawTexture', win, targtext,[],screenoffset-testcursor1_back(1,[3,4,1,2]),[],[],[],sel_aimingtargcol);
                    end
                ...
                %%save the data in experiment B and C
                %%%move_out direction
                if mod(ii,2)==0
                    aimingtagpos_x_out(ii-1)=testcursorx_out;
                    aimingtagpos_y_out(ii-1)=testcursory_out;
                    aimingangle1(t)=save_angle_out(testcursorx_out,testcursory_out,xstart,ystart);
                end
                if aimingangle1(t)==2                         % save the position of the cursor witho 0 in the vector
                    t=t-1;
                end
                ...
                if mod(ii,2)~=0
                    ...
                    aimingangle2(num)=save_angle_back(testcursorx_back,testcursory_back,xstart,ystart);
                end
                aimingangle2(1)=1;
                % save the position of the cursor witho 0 in the vector
                if aimingangle2(num)==2
...
% 
% function [angle_tag_back] = save_angle_back(testcursorx_back,testcursory_back,xstart,ystart)
% % save the  position of the aiming target when move_back
...
% return
%         
 function [aiming_tgt_angle_back] = save_angle_back(testcursorx_back,testcursory_back,xstart,ystart) 
        aiming_tgt_angle_back=atan2( diff([ystart,testcursory_back]), diff([xstart,testcursorx_back]))*180/pi;
        if aiming_tgt_angle_back<185&&aiming_tgt_angle_back>175
            aiming_tgt_angle_back=aiming_tgt_angle_back-90;
        elseif aiming_tgt_angle_back<5&&aiming_tgt_angle_back>-5
            aiming_tgt_angle_back=aiming_tgt_angle_back-90;
        elseif aiming_tgt_angle_back<-85.0&&aiming_tgt_angle_back>-95.0
            aiming_tgt_angle_back=aiming_tgt_angle_back+90;
        elseif aiming_tgt_angle_back>-85.0&&aiming_tgt_angle_back<-0.50
            aiming_tgt_angle_back=-aiming_tgt_angle_back-90;
        elseif aiming_tgt_angle_back<-95
            aiming_tgt_angle_back=-aiming_tgt_angle_back-90;
        end
return
